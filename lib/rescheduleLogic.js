...
async function trySwap(interview) {
  const dayOfWeek = (interview.dayNumber - 1) % 5; // 0=Mon ... 4=Fri
  ...
  const currentWeekEnd = Math.ceil(interview.dayNumber / 5) * 5;

  // Find all future scheduled interviews on the same weekday + time
  ...
  for (const future of futureCandidates) {
    ...
    if ((future.dayNumber - 1) % 5 !== dayOfWeek) continue;
...
function backfillVacatedSlot(vacatedInterview, pool) {
  const dayOfWeek = DAYS_OF_WEEK[(vacatedInterview.dayNumber - 1) % 5];
...
export async function smartReschedule(interviewId, reason = 'Candidate unavailable') {
  try {
    ...
    const interview = await prisma.interview.findUnique({
      where: { id: interviewId },
      include: {
        candidate: { select: { id: true, name: true, availability: true } },
    ...
    const currentWeek    = Math.ceil(interview.dayNumber / 5);
    ...
    const pendingCandidates = await prisma.candidate.findMany({
      ...
      select: { id: true, name: true, rollNumber: true, availability: true },
    });
