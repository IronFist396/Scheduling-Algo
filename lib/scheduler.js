...
// Ordered list of all slot labels — used to compute slot ranges for weekend overrides
...
// Default weekend window applied when no per-date override exists
const DEFAULT_WEEKEND_START = '11:30AM-12:30PM';
const DEFAULT_WEEKEND_END   = '11:30PM-12:30AM';

/**
 * Returns the allowed slot set for a given calendar date.
 * On weekdays: all slots (no restriction).
 * On weekends: slots within [startSlot, endSlot] from the override map,
 *              or the default window if no override exists for that date.
 */
function getAllowedSlots(date, weekendOverrides) {
  const dayOfWeek = date.getUTCDay(); // 0=Sun, 6=Sat
  const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
  if (!isWeekend) return null; // null = no restriction (all slots allowed)

  const isoDate = date.toISOString().slice(0, 10);
  const override = weekendOverrides?.[isoDate];

  const startSlot = override?.startSlot ?? DEFAULT_WEEKEND_START;
  const endSlot   = override?.endSlot   ?? DEFAULT_WEEKEND_END;

  const startIdx = ALL_SLOTS_ORDERED.indexOf(startSlot);
  const endIdx   = ALL_SLOTS_ORDERED.indexOf(endSlot);

  if (startIdx === -1 || endIdx === -1) return null; // fallback: no restriction
...
function calculateScheduledTime(dayNumber, slot, startDate) {
  ...
  scheduledDate.setUTCDate(scheduledDate.getUTCDate() + (dayNumber - 1));
  ...
  if (timeInfo) {
    ...
    if (utcHours < 0) { utcHours += 24; scheduledDate.setUTCDate(scheduledDate.getUTCDate() - 1); }
...
function scheduleInterviews(
  ...
  maxDays = 999,
  weekendOverrides = {} // { "2026-03-07": { startSlot, endSlot }, ... }
) {
  if (spmcs.length < 2) {
  ...
  // Load counter per person — used to break ties in Tier 1 panel selection
  // so that work is spread across all (SPMC, reviewer) combinations evenly.
  ...
  function findPanelForCandidateOnDay(candidate, dayNumber) {
    ...
    // Compute calendar date for this day to check weekend override
    const calDate = new Date(startDate);
    calDate.setUTCDate(calDate.getUTCDate() + (dayNumber - 1));
    const allowedSlots = getAllowedSlots(calDate, weekendOverrides); // null = no restriction
    ...
    if (tier1.length > 0) {
      // Primary sort: earliest slot.
      // Tiebreaker: least-loaded panel (sum of assignment counts for spmc + partner)
      // — ensures work spreads across all (SPMC, reviewer) combinations.
      tier1.sort((a, b) => {
        const tA = parseSlotTime(a.slot);
        const tB = parseSlotTime(b.slot);
        const slotDiff = (tA.hours * 60 + tA.minutes) - (tB.hours * 60 + tB.minutes);
        if (slotDiff !== 0) return slotDiff;
        const loadA = (assignmentCount[a.spmc.id] || 0) + (assignmentCount[a.partner.id] || 0);
        const loadB = (assignmentCount[b.spmc.id] || 0) + (assignmentCount[b.partner.id] || 0);
        return loadA - loadB;
      });
      return tier1[0];
    }
  ...
  for (const { candidate, availabilityScore } of scoredCandidates) {
    ...
    for (let day = 1; day <= maxDays && !scheduled; day++) {
      ...
      assignmentCount[spmc.id] = (assignmentCount[spmc.id] || 0) + 1;
      assignmentCount[partner.id] = (assignmentCount[partner.id] || 0) + 1;
